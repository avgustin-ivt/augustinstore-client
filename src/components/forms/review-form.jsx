...
const ReviewForm = ({product_id}) => {
  ...
  const onSubmit = (data) => {
    if(!user){
      notifyError("Сначала авторизуйтесь!");
      return;
    }
    else {
      addReview({
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className="tp-product-details-review-input-wrapper">
        <div className="tp-product-details-review-input-box">
          <div className="tp-product-details-review-input">
            <textarea
            {...register("comment", { required: `Напишите свой отзыв!` })}
              id="comment"
              name="comment"
          ...
          <div className="tp-product-details-review-input-title">
            <label htmlFor="msg">Ваше мнение</label>
          </div>
      ...
      <div className="tp-product-details-review-btn-wrapper">
        <button type="submit" className="tp-product-details-review-btn">Опубликовать</button>
      </div>
    </form>
  );
};
